...
const CROSS_BROWSER_LINK_ERROR =
  "That link didn't work — this usually happens when it's opened in a different browser or app than the one you requested it from (e.g. an email app's built-in browser). Enter the code from the same email instead, or request a new link and open it in the same browser you're signing in from.";
...
function LoginForm() {
  ...
  async function handleEmailSignIn(e: React.FormEvent) {
    ...
    const { error } = await supabase.auth.signInWithOtp({
      email,
      options: { emailRedirectTo: `${window.location.origin}/journey/auth/callback?next=${encodeURIComponent(next)}` },
    });
    ...
    <div className="flex flex-1 items-center justify-center bg-background px-4 py-24">
      <div className="w-full max-w-sm rounded-2xl border border-slate/15 bg-background p-8 shadow-sm">
        ...
        <p className="mt-2 text-sm text-slate">
          Your journal entries and trip planning — same account as the Reminders
          Android app, so anything logged there shows up here too.
        </p>
        ...
        {status === "sent" || status === "verifying" ? (
          <>
            <p className="mt-6 rounded-lg bg-accent-light px-4 py-3 text-sm text-navy">
              Check <span className="font-medium">{email}</span> for a sign-in code
              (or a sign-in link).
            </p>
