...
### `productshot/` subroutes (`src/app/productshot/*`)

A distinct sub-product sharing the main Next.js app and Supabase project, with
its own login/session flow (`src/lib/auth/`). Every route below reads data via
one of the context-scoped `src/lib/supabase/*` clients unless noted otherwise.

| Route | Purpose | Backing lib |
|---|---|---|
| `login/` | sign-in page | `@/lib/supabase/client` |
| `auth/callback/` | OAuth/session callback (route handler) | `@/lib/supabase/server` |
| `dashboard/` | landing page after login, guest banner for unauthenticated visits | `@/lib/supabase/public`, `@/lib/articles/fetch-posts` |
| `teardowns/`, `teardowns/[slug]/` | product-teardown articles, list + detail | `@/content/teardowns.ts` (not `src/lib` — static content module) |
| `articles/`, `articles/[slug]/` | general article feed, list + detail | `@/lib/articles/fetch-posts` |
| `news/` | productshot's news feed | `@/lib/supabase/public`, `@/lib/supabase/client`; generated by `src/lib/news/generate.ts` |
| `shots/` | productshot's core content pipeline output | `@/lib/supabase/public`, `@/lib/supabase/client`; generated by `src/lib/shots/generate.ts` + `rotation.ts` |
