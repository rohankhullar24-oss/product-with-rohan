...
/**
 * Image/audio attachments for Notes, one AES-256-GCM encrypted file per
 * attachment under filesDir/note_media/ (same Keystore master key pattern as
 * [JournalMediaStore]). A [Note] only ever stores the filename, never a raw path.
 */
object NoteMediaStore {

    private const val DIR_NAME = "note_media"
    private const val PLAYBACK_CACHE_DIR = "note_playback"
    ...
    /** Reads a plaintext temp file (e.g. a camera capture or a finished drawing/recording) and stores it encrypted, then deletes the temp file. */
    ...
    /** Decrypts an attachment fully into memory — fine for images, avoid for large audio. */
     ...
     * ImageView/MediaPlayer, which need a real file path/bitmap. Reuses the
     * cached copy across repeated reads; call [clearPlaybackCache] when done
     * (e.g. the editor's onDestroy) so plaintext doesn't linger.
     */
